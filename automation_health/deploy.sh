...
*/10 * * * * cd /opt/home-intelligence && /usr/bin/python3 -m automation_health.collect >> /opt/home-intelligence/automation_health/cron.log 2>&1

10 minutes matches the tightest watched automation's own trigger interval
(weathercloud_upload). No env file, no systemd unit -- the data source is
`docker logs homeassistant` (jramsey is already in the docker group), not
the HA REST API, and the textfile write goes through jramsey's existing
passwordless sudo (see exporter.py) rather than a new credential.
